...
class Strategy(Node):
    ...
    # def comm_cb(self, msg):
    #     self.get_logger().info(f'rm_comm got: {msg.comm}')
    #     self.ys_state=msg.comm
    #     if msg.comm==0:
...
def main(args=None):
    ...
    while rclpy.ok():
        ...
        if node.game_code=='play':
            ...
            # if kicker!=node.kicker:
            #     if time.time()-node.change_time>1: # 超过1秒才能切换
            #         node.get_logger().info(f'kicker changed from {node.kicker} to {kicker}')
            #         node.kicker=kicker
            #         node.change_time=time.time()
            #     else:
            #         node.get_logger().info(f'change too fast in {time.time()-node.change_time}s')
            node.kicker=kicker
            ...
            if in_position(node.p[node.kicker], node.target_pose[node.kicker], 0.16, 0.16): # 踢球不一定要踢准
        ...
        for p, tp in zip(node.p, node.target_pose):
            if i ==node.kicker:
                node.rel_cmd[i]=trans_relative_co(p, tp+np.array([1.8*node.v[0][0], 1.8*node.v[0][1], 0]))
