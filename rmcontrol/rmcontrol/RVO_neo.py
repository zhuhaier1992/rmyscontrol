    ...
    for i in range(len(X)):
        ...
        for j in range(len(X)):
            if i != j:
                ...
                if 2 * (ri+rj) > dist_BA:
                    dist_BA = 2 * (ri+rj)
                theta_BAort = asin(2 * (ri+rj) / dist_BA)
                ...
                RVO_BA = [transl_vB_vA, bound_left, bound_right, dist_BA, 2 * (ri+rj)]
...
def intersect(pA, vA, RVO_BA_all):
    ...
    suitable_V = []
    unsuitable_V = []
    for theta in numpy.arange(0, 2 * PI, 0.1):  # 68 cycle
        for rad in numpy.arange(0.02, norm_v + 0.02, norm_v / 5.0): # norm_v = 1 --> 50 cycle
            new_v = [rad * cos(theta), rad * sin(theta)]
            suit = True
            for RVO_BA in RVO_BA_all: # 5 cycle
                p_0 = RVO_BA[0]
                left = RVO_BA[1]
                right = RVO_BA[2]
                dif = [new_v[0] + pA[0] - p_0[0], new_v[1] + pA[1] - p_0[1]]  # 3 * arc sin    
